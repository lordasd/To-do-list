...
void Todolist::create_list(Todolist& list)
{
    ...
    if(list.todo_list.capacity() < num_of_tasks)
    {
        ...
        std::cout << "10 available tasks for use.\n";
    //Input tasks
    for(size_t tasknum = 0; tasknum < num_of_tasks; ++tasknum)
    {
        std::cout << "Input " << tasknum << " out of " << num_of_tasks << ": ";
...
void Todolist::show_list(Todolist& list)
{
    ...
    while(true)
    {
        ...
        if(input == 1)
        {
            ...
            std::cout << "Invalid choice. Choices:\n";
    }
}
...
void Todolist::load_save_file(Todolist& list)
{
    std::ifstream inp;
